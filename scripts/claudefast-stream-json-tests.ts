/**
 * claudefast stream-json smoke tests.
 *
 * This is intentionally not a normal unit test: it verifies the real Claude
 * Code stream-json protocol, hook lifecycle events, and filesystem effects.
 *
 * It runs:
 * 1. `claudefast -h` to prove the wrapper is present and supports the flags.
 * 2. A warn-level Write scenario that should allow a file containing axios and
 *    show the TeamAgent ASCII rule card in the PreToolUse hook response.
 * 3. A block-level JSON hard-rule scenario that should deny a filesystem write
 *    and leave the original JSON file unchanged.
 */
...
const outRoot = path.join(repoRoot, "scripts", "out", "claudefast-stream-json", timestamp());
...
async function runScenario(args: {
  ...
  settingsPath: string;
  prompt: string;
  checks: (events: JsonObject[], stdout: string) => ScenarioResult["checks"];
  ...
  const checks = args.checks(events, res.stdout);
...
async function main(): Promise<number> {
  ensureDir(outRoot);
  console.log("claudefast stream-json smoke tests");
