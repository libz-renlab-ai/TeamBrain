...
- **Build steps** (in order):
  ...
  6. **Stage release artifacts** into `/tmp/release-stage/`: `dist/`, `package.json`, `postinstall.mjs`, `install.sh` (P4 version, chmod 0755), `install-legacy.sh` (chmod 0755), `install.sh.sha256` (regenerated by `gen-sha256.sh`), `release-meta.json`
  7. **Create GitHub Release (idempotent)**: check if tag exists via `gh release view`; if not, `gh release create` uploading `teamagent-${TAG}.tgz`, `teamagent-${TAG}.tgz.sha256`, `release/install.sh.sha256`
...
- **Staging structure** (`/tmp/release-stage/` layout):
  ...
  ├── install.sh.sha256   # freshly generated by gen-sha256.sh
...
**Scope**: Out-of-scope for issue #121. This PR is explicitly about delivering the P4 installer and the release pipeline — not about fixing the `pipe | bash` self-verify path. The `SAFE_MODE=1` default already discourages pipe-to-bash usage; the bug only surfaces in `--auto` mode piped from curl.

**Action**: Raise as a follow-up `bug` issue after this PR merges. Tag `bug`, `installer`, `p4`, reference line 150 of `install.sh.draft` (post-Worker-B copy = `release/install.sh` line ~150). Proposed fix: detect pipe-to-bash scenario (`[ ! -t 0 ] && [ "$0" = "bash" -o "$0" = "/bin/bash" ]`) and skip self-verify in that case, emitting a warning instead.
