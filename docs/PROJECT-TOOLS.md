...
| **`FIXEDFLOW`** | 仓库唯一允许的 issue → PR → merge 工作流：<50 字 issue + grill 评论 + `grill-ready` label 触发本地 mainpi 全自动跑 step 3-5；非此模板的 issue 一律 close；详见 `docs/FIXEDFLOW.md`（取代已归档的 `docs/HOW-TO-ISSUE.md`） |
