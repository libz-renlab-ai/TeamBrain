...
5. **Squash-merge only.** `gh pr merge <N> --squash --delete-branch`. Never `--merge`, never `--rebase`. User-level memory `feedback_squash_only_merge.md` and `docs/POSTPR.md` "After `/review` PASS" both pin this.
