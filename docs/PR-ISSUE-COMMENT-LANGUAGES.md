...
CEO-duck proof-of-work image comments for already merged PRs are documented in `docs/PR-CEO-DUCK-PROOF-COMMENTS.md`: they are Chinese Bucket (c) human-facing comments with marker `teambrain-pr-ceo-duck-proof:image:v1`, backed by the imagehost-only repo `LiuShiyuMath/teambrain-pr-ceo-duck-proof-images`.
